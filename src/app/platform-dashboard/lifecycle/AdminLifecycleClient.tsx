...
import { fetchSchoolBackupData, verifyMasterAdminPassword, executeAdminYearEndWipe, executeAdminSystemRestore } from '@/app/actions/admin-lifecycle-actions'

export default function AdminLifecycleClient({ adminEmail, schools }: any) {
  ...
  // Map schools for the Custom Dropdown
  ...
  const handleGenerateBackup = async () => {
    ...
    try {
      const rawBackupData = await fetchSchoolBackupData(String(selectedSchoolId))
      const schoolName = schools.find((s: any) => s.id === selectedSchoolId)?.name || 'Institution'
      ...
      link.href = url
      link.setAttribute('download', `Master_${schoolName.replace(/\s+/g, '_')}_Archive_${new Date().getFullYear()}.zip`)
      ...
      alert("Failed to generate backup archive.")
    } finally {
  ...
  const handleWipe = async () => {
    setWipeError('')
    if (!password) return setWipeError('Master password is required.')
    
    setIsWiping(true)
    try {
      const authCheck = await verifyMasterAdminPassword(adminEmail, password)
      if (!authCheck.success) throw new Error(authCheck.error)

      const wipeResult = await executeAdminYearEndWipe(String(selectedSchoolId))
      if (!wipeResult.success) throw new Error(wipeResult.error)

      alert("Academic Year Data successfully wiped for the selected school.")
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!window.confirm("WARNING: This will inject historical data into the live database for this school. Proceed?")) {
    ...
    try {
      ...
      const restoreResult = await executeAdminSystemRestore(String(selectedSchoolId), parsedData)
  ...
  // Reset the flow if a new school is selected
  const handleSchoolChange = (val: string | number) => {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 font-sans text-stone-900 w-full">

      {/* Institution Selector */}
      <div className="bg-white rounded-sm border border-stone-200 p-6 md:p-8 shadow-sm relative z-50">
        <label className="block text-[10px] font-bold text-[#6b4c9a] tracking-widest uppercase mb-3">Select Target Institution</label>
        <Dropdown 
          options={schoolOptions}
          value={selectedSchoolId}
          onChange={handleSchoolChange}
          placeholder="-- Search & Select an Institution --"
          hasSearch={true}
        />
      </div>
      ...
      {/* Action Widgets */}
      {selectedSchoolId && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500 relative z-10 w-full">
          ...
          <div className="bg-white rounded-sm border border-stone-200 shadow-sm flex flex-col h-full">
            <div className="p-6 md:p-8 border-b border-stone-200 bg-[#fcf8f8]">
              <h2 className="text-lg font-semibold text-[#b4483e] uppercase tracking-wide">Force Rollover Wipe</h2>
              <p className="text-xs font-medium text-stone-600 mt-2">
                Permanently erase all academic records for the selected institution.
              </p>
            </div>
            ...
            <div className="p-6 md:p-8 flex-grow flex flex-col space-y-8 bg-white">
              ...
              <div>
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <p className="text-xs font-bold tracking-widest text-stone-800 uppercase">Master Admin Backup</p>
                </div>
                <button 
                  onClick={handleGenerateBackup}
                  disabled={isGenerating}
                  className="w-full bg-white border border-[#dad3e3] text-[#6b4c9a] px-5 py-3.5 rounded-sm text-xs uppercase tracking-widest font-bold hover:bg-[#fbf9fc] transition-colors shadow-sm disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  ...
                  {isGenerating ? 'Assembling Archive...' : 'Download Hybrid Archive (.zip)'}
                </button>
              </div>
              ...
              <div className={`transition-opacity duration-300 ${hasDownloaded ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
                ...
                {!hasDownloaded ? (
                  ...
                  <div className="space-y-4 bg-[#fcf8f8] border border-[#b4483e]/20 p-5 rounded-sm">
                    <div>
                      <label className="block text-[10px] font-bold uppercase tracking-wider text-stone-600 mb-2">Master Admin Password</label>
                      <input 
                        type="password" 
          ...
          <div className="bg-white rounded-sm border border-stone-200 shadow-sm flex flex-col h-full">
            <div className="p-6 md:p-8 border-b border-stone-200 bg-[#fbf9fc]">
              <h2 className="text-lg font-semibold text-[#6b4c9a] uppercase tracking-wide">Force Restoration</h2>
              <p className="text-xs font-medium text-stone-600 mt-2">
                Inject historical backup data directly into the selected school's live database.
              </p>
            </div>
            ...
            <div className="p-6 md:p-8 flex-grow flex flex-col bg-white">
              <p className="text-sm font-medium text-stone-600 mb-6 leading-relaxed">
                Upload a previously generated <span className="font-bold text-stone-900">Hybrid Archive (.zip)</span>. Ensure it corresponds to the correct institution to prevent cross-contamination.
              </p>
