...
export default function SetupForms({ schoolId, currentHolidays, classes, subjects, classSubjects }: any) {
  ...
  // Universal handler for standard form submissions with alerts & optional callback
  ...
  // Delete Class with confirmation and alert
  ...
  // Remove Subject with confirmation and alert
  ...
  return (
    <div className="space-y-12 lg:space-y-16 font-sans pb-16 text-stone-900 max-w-7xl mx-auto">
      ...
      <section className="bg-white rounded-sm shadow-sm border border-stone-200 p-6 md:p-8">
        ...
        {/* Classes Catalog Grid (Optimized for 5 or 6 items uniformly) */}
        {classes.length === 0 ? (
                  ...
                  {editingClassId === cls.id ? (
                    <div className="p-6 bg-[#fbf9fc] flex-grow flex flex-col">
                      <div className="mb-5 pb-3 border-b border-[#dad3e3]">
                        ...
                        <p className="text-[11px] text-stone-600 font-medium mt-1">Modify the class name or change the daily periods.</p>
                      </div>
                      <form onSubmit={(e) => handleActionWithAlert(e, updateClassRecord, "Class updated successfully!")} className="space-y-4 flex-grow flex flex-col">
                        ...
                        <div>
                          ...
                          <input 
                            ...
                            required 
                            className="w-full p-3 bg-white border border-stone-300 rounded-sm text-sm font-medium text-stone-900 focus:outline-none focus:border-[#6b4c9a] focus:ring-1 focus:ring-[#6b4c9a]" 
                          />
                        </div>
                        ...
                        <div>
                          ...
                          <input 
                            type="number" 
                            name="periodsPerDay" 
                            defaultValue={cls.periods_per_day} 
                            required 
                            className="w-full p-3 bg-white border border-stone-300 rounded-sm text-sm font-medium text-stone-900 focus:outline-none focus:border-[#6b4c9a] focus:ring-1 focus:ring-[#6b4c9a]" 
                          />
                        </div>
                        ...
                        <div className="flex gap-3 pt-4 mt-auto">
                          <button type="submit" className="flex-1 bg-[#6b4c9a] text-white text-[11px] uppercase tracking-widest font-bold px-4 py-3 rounded-sm hover:bg-[#5a3f82] transition-colors shadow-sm">Save Changes</button>
                    ...
                    <>
                      <div className="bg-stone-50 p-6 border-b border-stone-200 flex justify-between items-start group relative min-h-[105px]">
                        ...
                        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-stone-50 pl-2">
                          <button onClick={() => setEditingClassId(cls.id)} className="bg-white border border-stone-200 text-stone-500 hover:text-[#6b4c9a] hover:border-[#6b4c9a] p-2 rounded-sm transition-colors shadow-sm" title="Edit Class Details">
                        ...
                        {mappedSubjects.length > 0 ? (
                          <div className="flex flex-wrap gap-2.5">
                            {mappedSubjects.map((sub: any) => (
                              <span key={sub.id} className="inline-flex items-center gap-2 bg-white text-stone-800 border border-stone-300 text-xs font-semibold px-3 py-2 rounded-sm tracking-wide hover:border-[#b4483e] hover:bg-[#fcf8f8] transition-colors group/tag">
                                {sub.name}
                                <button 
                          ...
                          <p className="text-sm font-medium text-stone-500 italic bg-stone-50 p-4 rounded-sm border border-stone-200 border-dashed text-center">No subjects added to this class yet.</p>
                        )}
                      </div>

                      {/* Add Subject Subform */}
                      <div className="p-6 bg-stone-50 border-t border-stone-100 mt-auto">
                        <form onSubmit={(e) => handleActionWithAlert(e, addSubjectToClass, "Subject added successfully to class!")} className="flex gap-3">
                          <input type="hidden" name="schoolId" value={schoolId} />
                          <input type="hidden" name="classId" value={cls.id} />
                          <input 
                            type="text" 
                            ...
                            required 
                            className="flex-grow p-3 border border-stone-300 rounded-sm text-sm font-medium bg-white text-stone-900 placeholder:text-stone-400 focus:outline-none focus:border-[#6b4c9a] focus:ring-1 focus:ring-[#6b4c9a] transition-all shadow-sm" 
                          />
                          <button 
                            type="submit" 
                            disabled={isSubmitting} 
                            className="bg-white border border-stone-300 text-stone-800 px-5 py-3 rounded-sm text-[11px] uppercase tracking-widest font-bold hover:border-[#6b4c9a] hover:text-[#6b4c9a] transition-colors disabled:opacity-50 shadow-sm shrink-0"
                          >
                            Add
                          </button>
                        </form>
